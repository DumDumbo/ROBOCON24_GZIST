...
#include "robot_control_demo_affiliated.h"
...
{
    // 初始化节点
    ros::init(argc, argv, "robot_control_demo_affiliated");
    //实例化控制器对象
...
void RcController::handleSearching(bool& aligned_with_ball, ros::Rate& rate)
{
    ...
    if(RobotStatus == "GrainSearch"){
        ...
        else if(ballColor == "Red" || ballColor == "Blue"){
            // 速度设置 
            cmd_vel_msg.linear.x = 0.0;
            cmd_vel_msg.linear.y = 0.0;
            cmd_vel_msg.angular.z = 0.0;
        ...
        // 判断有无对准球
        if (!aligned_with_ball && !ResetMachine && target_pos.Class == "PurpleBall"){//没对准时，旋转进行对准
            //计算速度 
...
void RcController::handlePreHoming(float& laserD, ros::Rate& rate)
{
    ...
    // 计算速度
    PIDOutputX = computeLimitedPIDOutput('x', KeepX, robot_pos.posX, 0.4);
    PIDOutputY = computeLimitedPIDOutput('y', KeepY, laserD, 0.6);
    PIDOutputYaw = computeLimitedPIDOutput('w', homing_yaw, Imu_Data.Euler.yaw, 2.0);
    // 速度赋值
    cmd_vel_msg.linear.x = PIDOutputX;
    cmd_vel_msg.linear.y = (areaColor == "Red") ? -PIDOutputY : PIDOutputY;;
    cmd_vel_msg.angular.z = -PIDOutputYaw;
...
void RcController::handleHoming(float& laserD, ros::Rate& rate)
{
    std::cout << "RobotStatus: " << RobotStatus << std::endl;
    //计算速度 
    PIDOutputX = computeLimitedPIDOutput('x', KeepX, Laser_Ranging.LaserRear, 0.5);
    PIDOutputY = computeLimitedPIDOutput('y', KeepY, laserD, 0.5);
    PIDOutputYaw = computeLimitedPIDOutput('w', homing_yaw, Imu_Data.Euler.yaw, 1.0);
    // 速度赋值
    cmd_vel_msg.linear.x = PIDOutputX;
    cmd_vel_msg.linear.y = (areaColor == "Red") ? PIDOutputY : -PIDOutputY;
    cmd_vel_msg.angular.z = -PIDOutputYaw;
    // 更新状态条件
    if (0.026 < Laser_Ranging.LaserRear && Laser_Ranging.LaserRear <= 0.10){
        KeepX = 1.9;
        KeepY = 1.66;
...
void RcController::UnifiedControl()
{
    ...
    while (ros::ok()){
        if (RobotStatus == "Initialized") {
            ...
            // 调转车头(红方逆时针90°旋转，蓝方顺时针90°)
            homing_yaw = (areaColor == "Red") ? homing_yaw+90 : homing_yaw-90;
            if(homing_yaw > 180.0){homing_yaw -= 360.0;}
            else if (homing_yaw < -180.0) {homing_yaw += 360.0;}
