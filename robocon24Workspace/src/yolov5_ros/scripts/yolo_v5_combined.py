...
class Yolo_Dect:
    def __init__(self):
        ...
        image_topic_2 = rospy.get_param('~image_topic_2', '/camera_335/color/image_raw')
        ...
        self.color_sub_2 = rospy.Subscriber(image_topic_2, Image, self.image_callback_2,
                                            queue_size=1, buff_size = 52428800)
        # output publishers
    ...
    def image_callback(self, image):
        if self.RobotStatus == "2":
            ...
            # time3 = time2 - time1
            # rospy.loginfo(f"Inference time: {time3.to_sec()} seconds")
            ...
            self.boundingBoxes = BoundingBoxes()
            self.boundingBoxes.header = self.imageRear.header
            self.boundingBoxes.image_header = self.imageRear.header
            self.getImageStatus = True
            # 将Image话题的一维数据转化重新排列组合成三维的Numpy数组（height，width， channels）
            # 若话题图形本身为rgb8的编码，无须转换。若不是请转换为rgb8
            # 可使用print(image.encoding)进行查看, 因本相机为rgb8,故无须转换
            self.color_image = np.frombuffer(self.imageRear.data, dtype=np.uint8).reshape(
                image.height, image.width, -1)
            # 获取推理结果
            # time1 = rospy.Time.now()
            results = self.model(self.color_image)
    ...
    def dectshow(self, org_img, boxs, height, width):
        ...
        for box in boxs:
            ...
            boundingBox.Class =self.class_names[box[-1]]
        ...
        if count ==0:
            ...
            boundingBox0.probability = np.float64(0)
            boundingBox0.xmin =  np.int64(0)
            boundingBox0.ymin =  np.int64(0)
            boundingBox0.xmax =  np.int64(0)
            boundingBox0.ymax =  np.int64(0)
            boundingBox0.num = np.int16(count)
            boundingBox0.Class = "None"

            self.boundingBoxes.bounding_boxes.append(boundingBox0)
            self.position_pub.publish(self.boundingBoxes)
...
def main():
    rospy.init_node('yolo_v5_combined', anonymous=True)
    yolo_dect = Yolo_Dect()
    rospy.spin()
