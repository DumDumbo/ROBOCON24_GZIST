...
class CameraCalibrator(object):
    def __init__(self, image_size: tuple):
        ...
        self.matrix = np.zeros((3, 3), float)
        self.new_camera_matrix = np.zeros((3, 3), float)
        self.dist = np.zeros((1, 5))
        self.roi = np.zeros(4, int)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=str, default='1920*1080', help='width*height of image')
    parser.add_argument('--mode', type=str, default='calibrate', choices=['calibrate', 'rectify'], help='to calibrate '
                                                                                                        'or rectify')
    parser.add_argument('--square', type=int, default=13, help='size of chessboard square, by mm')
    parser.add_argument('--corner', type=str, default='9*6', help='width*height of chessboard corner')
    ...
    if args.mode == 'calibrate':
        if not args.corner or not args.square:
            ...
            exit(-1)
        corner = tuple(int(i) for i in args.corner.split('*'))
